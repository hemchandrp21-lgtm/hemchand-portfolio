...
function Footer() {
  ...
  return (
    <footer className="w-full bg-[#f8f9fa] text-zinc-900 border-t border-zinc-200 py-12 px-6 lg:px-12 select-none">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
        {/* Left Signature */}
        <div className="flex flex-col sm:flex-row items-center gap-4 text-center sm:text-left">
          <Link to="/" className="text-base font-display font-bold uppercase tracking-widest text-zinc-900">
            HEMCHAND P.
          </Link>
        ...
        <div className="flex items-center gap-6">
          {socials.map((s) => (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="text-xs font-mono text-zinc-600 hover:text-amber-600 transition-colors uppercase tracking-wider font-semibold"
            >
              {s.name}
            </a>
          ))}
